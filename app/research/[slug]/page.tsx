...
const components:PortableTextComponents={
  ...
  types:{dataTable:({value})=><div className="data-table-wrap"><table className="data-table"><thead><tr>{value.columns?.map((c:string,i:number)=><th key={i}>{c}</th>)}</tr></thead><tbody>{value.rows?.map((r:any)=><tr key={r._key}>{r.cells?.map((c:string,i:number)=><td key={i}>{c}</td>)}</tr>)}</tbody></table>{value.caption&&<p style={{fontSize:'.82rem',color:'#667477'}}>{value.caption}</p>}</div>}
}
...
{p.sources?.length?<section className="panel sources"><div className="eyebrow">References</div><h2>Sources and data notes</h2><ol>{p.sources.map(s=><li key={s._key}>{s.citation} {s.url&&<a href={s.url}>Source ↗</a>}</li>)}</ol>{p.sourceNote&&<p><strong>Source note:</strong> {p.sourceNote}</p>}</section>:null}
</article></main>}
